...
void main() {
    //gl_FragColor = texture2D(colorMap, vUv) * time;
    ...
    vec3 lookup_table_color = texture2D(colorMap, vec2(inverse_dot_view, 0.0)).rgb;

    gl_FragColor.rgb = textureCube(texture_reflection, n_reflection).rgb * lookup_table_color * 3.5;
    gl_FragColor.a = 1.0;
